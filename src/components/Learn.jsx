...
console.log("User data or email is missing!");
console.log(userData.email)
  ...
  async function updateUserLanguageByEmail(email) {
    try {
      ...
      return userDoc.languageLearn; // Return the language field for comparison
    } catch (error) {
  ...
  useEffect(() => {
    const updateAndFetchData = async () => {
      ...
      const userLanguage = await updateUserLanguageByEmail(userData.email);
  ...
  }, [userData.email]); // Add props.email to the dependency array to trigger updates when it changes
